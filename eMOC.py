...
from Thead_server import ProjectServerThread, Variables
from Thead_client import *

__version__ = "1.0"
__version_date__ = "2018-06-12"

if platform.python_version() < "3.5":
    logging.critical("Requires Python 3.5+! You are using v. {}")
    sys.exit()
    ...
    def run(self):
        while self.exiting is False:
            if sum(os.path.getsize(self.tempdir + f) for f in os.listdir(self.tempdir)
                   if "eMOC@" in f and os.path.isfile(self.tempdir + f)) > self.ffmpeg_cache_dir_max_size:
                fl = sorted((os.path.getctime(self.tempdir + f), self.tempdir + f) for f in os.listdir(self.tempdir)
                            if "eMOC@" in f and os.path.isfile(self.tempdir + f))
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    checkForNewVersion = False  # if True eMOC will check for new version every 15 days
    ...
    def __init__(self, availablePlayers, ffmpeg_bin, parent=None):
        ...
        self.actionStop_observation.setIcon(QIcon("stop.png"))
        ...
        self.lbLogoeMOC.setPixmap(
            QPixmap(datadir + "/logo_emoc.png"))  # imagem da logo que aparece na tela principal da aplicação
        self.lbLogoeMOC.setScaledContents(False)
        self.lbLogoeMOC.setAlignment(Qt.AlignCenter)
        ...
        self.dwObservationsGroup.setVisible(False)
    ...
    def menu_options(self):
        ...
        self.actionStop_observation.setEnabled(self.playerType == VLC)
    ...
    def connections(self):
        ...
        self.actionOpen_server.triggered.connect(self.open_thead_server)
        self.actionEnter_colaboration.triggered.connect(self.colaboration_project)
        ...
        self.actionStop_observation.triggered.connect(self.stop_observation)
                    ...
                    for idx in sorted_keys(behav[MODIFIERS]):
                        if behav[MODIFIERS][idx]["name"]:
                            modifiers += "<br>Name: {}<br>Type: {}<br>".format(behav[MODIFIERS][idx]["name"]
                                                                               if behav[MODIFIERS][idx][
                                "name"] else "-",
                                                                               MODIFIERS_STR[
                                                                                   behav[MODIFIERS][idx]["type"]])
    ...
    def getProject(self):
        return copy.deepcopy(self.pj) or {}

    def colaboration_project(self):
        colaborationWindow = colaboration.Colaboration()

        def loadFile(content):
            ...
            def choose_for_existing_file(instance, filename):
                if instance == "Cancelar":
                    return
                if instance == "Sobrescrever":
                    save_project_file(filename, content)
                    return
                if instance == "Renomear":
                    save_project_file("{}.{}.eMOC".format(filename,
                                                           datetime.datetime.now().isoformat("_").split(".")[0].replace(
                                                               ":", "")), content)
                    return
            ...
            if content:
                ...
                if "project_name" in decoded and decoded["project_name"]:
                    filename = decoded["project_name"] + ".eMOC"
                else:
                    filename = "NONAME_" + datetime.datetime.now().isoformat("_").split(".")[0].replace(":",
                                                                                                        "") + ".eMOC"

                if os.path.isfile(filename):
                    file_exists = dialog.MessageDialog(programName, "A observação já existe. O que deseja fazer?",
                                                       ["Sobrescrever", "Renomear", "Cancelar"])
                    choose_for_existing_file(file_exists, filename)
                else:
                    save_project_file(filename, content)
            else:
                return
        ...
        def add_aplication():
            url_project = colaborationWindow.edit_add_url.text()
            colaborationWindow.list_url.addItem(url_project)

            if url_project != '':
                self.servers.append(url_project)

            colaborationWindow.edit_add_url.setText("")
            colaborationWindow.edit_add_url.setFocus()

        def remove_aplication():
            self.servers.remove(colaborationWindow.list_url.currentItem().text())

            colaborationWindow.list_url.takeItem(
                colaborationWindow.list_url.row(
                    colaborationWindow.list_url.currentItem()
                )
            )

        def send_signal():
            for pyro in self.servers:
                filelist = client_receive_project(pyro)

                for filename in filelist:
                    filedate = re.search(".*\.eMOC$", filename)
                    if filedate:
        ...
        colaborationWindow.btn_add.clicked.connect(add_aplication)
        colaborationWindow.btn_remove.clicked.connect(remove_aplication)
        colaborationWindow.btn_start.clicked.connect(send_signal)

        colaborationWindow.exec_()

    def open_thead_server(self):
        self.IsServidor = True

        def receive_signal(msg_dict):
            display = QDialog(self)
            display.setWindowTitle("URL Servidor")
            display.setWindowModality(Qt.ApplicationModal)
            display.resize(200, 50)

            self.servers.append(msg_dict["URL"])

            label = QLabel("URL SERVIDOR: {}".format(msg_dict["URL"]), display)
            label.setAlignment(Qt.AlignCenter)

            display.exec_()


        self.server_thread = ProjectServerThread(self.pj)
    ...
    def ffmpeg_process(self, action):
        ...
        if self.ffmpeg_process_ps:
            QMessageBox.warning(self, programName, "eMOC is already running a ffmpeg process...")
            return

        fn = QFileDialog(self).getOpenFileNames(self, "Select one or more media files to process", "",
                                                "Media files (*)")
        fileNames = fn[0] if type(fn) is tuple else fn

        if fileNames:
            if action == "reencode_resize":
                horiz_resol, ok = QInputDialog.getInt(self, "", (
            ...
            for file_name in fileNames:
                ...
                if action == "rotate":
                    fn = "{input}.rotated{rotation}.avi".format(input=file_name,
                ...
                for subject in plot_parameters["selected subjects"]:
                    ...
                    for behavior in plot_parameters["selected behaviors"]:
                        ...
                        for idx, row in enumerate(rows):
                            ...
                            start = round(row["occurence"] - timeOffset - float2decimal(sum(duration1[0:mediaFileIdx])),
                                          3)
                            if start < timeOffset:
                                start = Decimal("0.000")
                            ...
                            if POINT in self.eventType(behavior).upper():
                                ...
                                stop = round(
                                    row["occurence"] + timeOffset - float2decimal(sum(duration1[0:mediaFileIdx])), 3)

                                ffmpeg_command = ffmpeg_extract_command.format(
                                    ffmpeg_bin=ffmpeg_bin,
                                    input_=project_functions.media_full_path(
                                        self.pj[OBSERVATIONS][obsId][FILE][nplayer][mediaFileIdx],
                            ...
                            if STATE in self.eventType(behavior).upper():
                                if idx % 2 == 0:
                                    ...
                                    if start > self.pj[OBSERVATIONS][obsId]["media_info"]["length"][
    ...
    def show_spectrogram(self):
        ...
        if self.playerType == LIVE:
            QMessageBox.warning(self, programName,
                                "The spectrogram visualization is not available for live observations")
            return
        ...
        if hasattr(self, "spectro"):
            ...
            if dialog.MessageDialog(programName, ("You choose to visualize the spectrogram during this observation.<br>"
                ...
                currentMediaTmpPath = tmp_dir + os.sep + os.path.basename(
                    url2path(self.mediaplayer.get_media().get_mrl()))
                ...
                self.spectro = plot_spectrogram.Spectrogram(
                    "{}.wav.0-{}.{}.{}.spectrogram.png".format(currentMediaTmpPath,
                                                               self.chunk_length,
            ...
            if self.pj[OBSERVATIONS][self.observationId][TYPE] in [MEDIA]:
                ...
                if mode == "start":
                    if not self.initialize_new_observation_vlc():
                        ...
                        self.menu_options()
                        return "Erro: problema com o carregamento da observação."
    ...
    def actionCheckUpdate_activated(self, flagMsgOnlyIfNew=False):
        """
        check eMOC web site for updates
        """
        try:
            versionURL = "http://www.eMOC.unito.it/static/ver4.dat"
            ...
            if versiontuple(lastVersion) > versiontuple(__version__):
                msg = ("""A new version is available: v. <b>{}</b><br>"""
                       """Go to <a href="http://www.eMOC.unito.it">"""
                       """http://www.eMOC.unito.it</a> to install it.""").format(lastVersion)
            else:
                msg = "The version you are using is the last one: <b>{}</b>".format(__version__)
            newsURL = "http://www.eMOC.unito.it/static/news.dat"
    ...
    def preferences(self):
        ...
        try:
            preferencesWindow.cbFrameBitmapFormat.setCurrentIndex(
                FRAME_BITMAP_FORMAT_LIST.index(self.frame_bitmap_format))
        except:
            preferencesWindow.cbFrameBitmapFormat.setCurrentIndex(
                FRAME_BITMAP_FORMAT_LIST.index(FRAME_DEFAULT_BITMAP_FORMAT))
        ...
        try:
            preferencesWindow.cbSpectrogramColorMap.setCurrentIndex(
                SPECTROGRAM_COLOR_MAPS.index(self.spectrogram_color_map))
        except:
            preferencesWindow.cbSpectrogramColorMap.setCurrentIndex(
                SPECTROGRAM_COLOR_MAPS.index(SPECTROGRAM_DEFAULT_COLOR_MAP))
        ...
        if preferencesWindow.exec_():
            ...
            if self.frame_resize != mem_frame_resize:
                ...
                for f in [x for x in os.listdir(self.imageDirectory)
                          if "eMOC@" in x and os.path.isfile(self.imageDirectory + os.sep + x)]:
                    try:
            ...
            if self.playMode == FFMPEG:
                ...
                if self.detachFrameViewer:
                    if hasattr(self,
                               "measurement_w") and self.measurement_w is not None and self.measurement_w.isVisible():
                        QMessageBox.warning(self, programName,
    ...
    def ffmpegTimerOut(self):
        ...
        frame_image_path = "{imageDir}{sep}eMOC@{fileName}_{frame:08}.{extension}".format(imageDir=self.imageDirectory,
                                                                                           sep=os.sep,
        ...
        if os.path.isfile(frame_image_path):
            ...
            if not os.path.isfile(frame_image_path):
                logging.warning("frame not found: {} {} {}".format(frame_image_path, frameCurrentMedia,
        ...
        if self.second_player():
            ...
            frame_image_path2 = "{imageDir}{sep}eMOC@{fileName}_{frame:08}.{extension}".format(
            ...
            if os.path.isfile(frame_image_path2):
                ...
                if not os.path.isfile(frame_image_path2):
                    logging.warning("frame not found: {} {} {}".format(frame_image_path2, frameCurrentMedia2,
            ...
            '''
            if "eMOC@{md5FileName}-{second}".format(md5FileName=md5FileName2,
            ...
            img2 = "{imageDir}{sep}eMOC@{fileName}-{second}_{frame}.{extension}".format(imageDir=self.imageDirectory,
                                                                                         sep=os.sep,
                                                                                         fileName=md5FileName2,
                                                                                         second=second2,
                                                                                         frame=frame2,
        ...
        if self.detachFrameViewer or self.second_player():  # frame viewer detached or 2 players
            ...
            self.frame_viewer1.lbFrame.setPixmap(
                self.pixmap.scaled(self.frame_viewer1.lbFrame.size(), Qt.KeepAspectRatio))
            if self.second_player():
                self.frame_viewer2.lbFrame.setPixmap(
                    self.pixmap2.scaled(self.frame_viewer2.lbFrame.size(), Qt.KeepAspectRatio))
    ...
    def distance(self):
        ...
        if self.detachFrameViewer:
            QMessageBox.warning(self, programName,
                                ("The geometric measurement is only available when the frames viewer is not detached\n"
    ...
    def initialize_new_observation_vlc(self):
        ...
        ok, msg = project_functions.check_if_media_available(self.pj[OBSERVATIONS][self.observationId], self.projectFileName)
        if not ok:
            QMessageBox.critical(self, programName, msg + ("<br><br>A observação será aberta no modo VISUALIZAÇÃO.<br>"
                                                           "Não será possível registrar eventos.<br>"
                                                           "Modifique o caminho da mídia para apontar um arquivo de mídia existente "
                                                           "para registrar eventos ou copiar arquivos de mídia no diretório do projeto eMOC."),
                                 QMessageBox.Ok | QMessageBox.Default,
                                 QMessageBox.NoButton)
        ...
        if (len(self.pj[OBSERVATIONS][self.observationId][FILE][PLAYER1]) > 1 and
                ...
                self.pj[OBSERVATIONS][self.observationId][FILE][PLAYER2]):
            QMessageBox.warning(self, programName, ("Ainda não é possível reproduzir uma segunda mídia quando mais mídia"
                                                    "são carregados no primeiro media player"))
            return False
        ...
        if PLAYER2 in self.pj[OBSERVATIONS][self.observationId][FILE] and \
            ...
            if useMediaFromProjectDirectory == YES:
                for idx, mediaFile in enumerate(self.pj[OBSERVATIONS][self.observationId][FILE][PLAYER2]):
                    self.pj[OBSERVATIONS][self.observationId][FILE][PLAYER2][idx] = (
        ...
        if ("visualize_spectrogram" in self.pj[OBSERVATIONS][self.observationId] and
            ...
            currentMediaTmpPath = tmp_dir + os.sep + os.path.basename(
                urllib.parse.unquote(url2path(self.mediaplayer.get_media().get_mrl())))
        ...
        # external data plot
        if PLOT_DATA in self.pj[OBSERVATIONS][self.observationId] and self.pj[OBSERVATIONS][self.observationId][
            PLOT_DATA]:
            ...
            for idx in self.pj[OBSERVATIONS][self.observationId][PLOT_DATA]:
                if count == 0:
                    ...
                    w1 = plot_data_module.Plot_data(data_file_path,
                                                    int(self.pj[OBSERVATIONS][self.observationId][PLOT_DATA][idx][
                                                            "time_interval"]),
                                                    str(self.pj[OBSERVATIONS][self.observationId][PLOT_DATA][idx][
                                                            "time_offset"]),
    ...
    def new_observation(self, mode=NEW, obsId=""):
        ...
        if mode == NEW and self.observationId:
            if dialog.MessageDialog(programName, "A observação atual será encerrada. Você quer continuar?", [YES, NO]) == NO:
                return
            else:
                self.close_observation()
        ...
        if mode == EDIT:
            ...
            # check date format for old versions of eMOC app
            try:
                import time
            ...
            try:
                ...
                logging.info("Nenhuma informação de vídeo/áudio")

            # offset
            if self.timeFormat == S:

                observationWindow.leTimeOffset.setText(
                    self.convertTime(abs(self.pj[OBSERVATIONS][obsId]["time offset"])))

                if "time offset second player" in self.pj[OBSERVATIONS][obsId]:
                    observationWindow.leTimeOffset_2.setText(
                        self.convertTime(abs(self.pj[OBSERVATIONS][obsId]["time offset second player"])))
            ...
            if "visualize_spectrogram" in self.pj[OBSERVATIONS][obsId]:
                observationWindow.cbVisualizeSpectrogram.setChecked(
                    self.pj[OBSERVATIONS][obsId]["visualize_spectrogram"])
                    ...
                    for idx2 in sorted_keys(self.pj[OBSERVATIONS][obsId][PLOT_DATA]):
                        ...
                        for idx3 in DATA_PLOT_FIELDS:
                            if idx3 == PLOT_DATA_PLOTCOLOR_IDX:
                                ...
                                combobox.setCurrentIndex(DATA_PLOT_STYLES.index(
                                ...
                                '''
                                observationWindow.tw_data_files.setItem(observationWindow.tw_data_files.rowCount() - 1,
                                                                        idx3,
                                                                        QTableWidgetItem(str(
                                                                            self.pj[OBSERVATIONS][obsId][PLOT_DATA][
                                                                                idx2][DATA_PLOT_FIELDS[idx3]])))
            ...
            if CLOSE_BEHAVIORS_BETWEEN_VIDEOS in self.pj[OBSERVATIONS][obsId]:
                observationWindow.cbCloseCurrentBehaviorsBetweenVideo.setChecked(
                    self.pj[OBSERVATIONS][obsId][CLOSE_BEHAVIORS_BETWEEN_VIDEOS])
        ...
        if rv:
            ...
            if mode == EDIT and new_obs_id != obsId:
                logging.info("Observação com id {} mudou para {}".format(obsId, new_obs_id))
            ...
            for r in range(observationWindow.twIndepVariables.rowCount()):
                ...
                if observationWindow.twIndepVariables.item(r, 1).text() == SET_OF_VALUES:
                    self.pj[OBSERVATIONS][new_obs_id][INDEPENDENT_VARIABLES][observationWindow.twIndepVariables.item(r, 0).text()] = observationWindow.twIndepVariables.cellWidget(
                        r, 2).currentText()
            ...
            if self.timeFormat == HHMMSS:
                self.pj[OBSERVATIONS][new_obs_id][TIME_OFFSET] = time2seconds(
                    observationWindow.teTimeOffset.time().toString(HHMMSSZZZ))
                self.pj[OBSERVATIONS][new_obs_id][TIME_OFFSET_SECOND_PLAYER] = time2seconds(
                    observationWindow.teTimeOffset_2.time().toString(HHMMSSZZZ))

            if self.timeFormat == S:
                self.pj[OBSERVATIONS][new_obs_id][TIME_OFFSET] = abs(Decimal(observationWindow.leTimeOffset.text()))
                self.pj[OBSERVATIONS][new_obs_id][TIME_OFFSET_SECOND_PLAYER] = abs(
                    Decimal(observationWindow.leTimeOffset_2.text()))
            ...
            if observationWindow.rbEarlier.isChecked():
                self.pj[OBSERVATIONS][new_obs_id][TIME_OFFSET_SECOND_PLAYER] = - self.pj[OBSERVATIONS][new_obs_id][
                    TIME_OFFSET_SECOND_PLAYER]
                ...
                for row in range(observationWindow.tw_data_files.rowCount()):
                    ...
                    for idx2 in DATA_PLOT_FIELDS:
                        if idx2 in [PLOT_DATA_PLOTCOLOR_IDX, PLOT_DATA_SUBSTRACT1STVALUE_IDX]:
                            self.pj[OBSERVATIONS][new_obs_id][PLOT_DATA][str(row)][
                                DATA_PLOT_FIELDS[idx2]] = observationWindow.tw_data_files.cellWidget(row,
                                                                                                     idx2).currentText()
            ...
            # for pyro in self.servers:
            #     client_sign_up(pyro, new_obs_id, self.pj[OBSERVATIONS][new_obs_id])
            ...
            if rv == 2:  # start
                self.observationId = new_obs_id
                # title of dock widget
                self.dwObservations.setWindowTitle("""Eventos para o observador "{}". """.format(self.observationId))
                ...
                self.menu_options()
                #self.send_data(self.observationId)

    def send_data(self, user):
        """
        send data from socket eMOC
        """
        self.dwObservationsGroup.setVisible(True)

        for pyro in self.servers:
            inicialization_colaboration(pyro, factory, self)
    ...
    def readConfigFile(self):
        ...
        iniFilePath = str(pathlib.Path(os.path.expanduser("~")) / ".eMOC")
        ...
        if os.path.isfile(iniFilePath):
            ...
            try:
                if settings.value("check_for_new_version") == None:
                    self.checkForNewVersion = (dialog.MessageDialog(programName,
                                                                    (
                                                                        "Allow eMOC to automatically check for new version and news?\n"
            ...
            if self.checkForNewVersion:
                if (settings.value("last_check_for_new_version") and
                        int(time.mktime(time.localtime())) - int(
                            settings.value("last_check_for_new_version")) > CHECK_NEW_VERSION_DELAY):
        ...
        else:  # no .eMOC file found
            # ask user for checking for new version
            self.checkForNewVersion = (
                    dialog.MessageDialog(programName, ("Allow eMOC to automatically check for new version?\n"
                                                       "(An internet connection is required)\n"
        ...
        iniFilePath = str(pathlib.Path(os.path.expanduser("~")) / ".eMOC_recent_projects")
        if os.path.isfile(iniFilePath):
            settings = QSettings(iniFilePath, QSettings.IniFormat)
    ...
    def eventType(self, code):
        ...
        for idx in self.pj[ETHOGRAM]:
            if self.pj[ETHOGRAM][idx]['key'] == code:
    ...
    def synthetic_time_budget(self):
        ...
        filediag_func = QFileDialog(self).getSaveFileNameAndFilter if QT_VERSION_STR[0] == "4" else QFileDialog(
        ...
        for obsId in selectedObservations:
                ...
                for behavior in plot_parameters["selected behaviors"]:
                    ...
                    if plot_parameters["include modifiers"]:
                        ...
                        for modifier in distinct_modifiers:
                            ...
                            cursor.execute((
                                "SELECT occurence FROM events WHERE observation = ? AND subject = ? AND code = ? AND modifiers = ? "
                                "ORDER BY observation, occurence"),
                                ...
                                if STATE in self.eventType(behavior).upper():
                                    if idx % 2 == 0:  # only 1 occurence of 2
                                        try:
                                            ...
                                            QMessageBox.critical(self, programName,
                                                                 "Error on observation <b>{}</b>".format(obsId))
            ...
            if len(selectedObservations) > 1:
                output_file_name = str(
                    pathlib.Path(pathlib.Path(plot_directory) / safeFileName(obsId)).with_suffix("." + file_format))
            else:
                output_file_name = ""

            ret = plot_events.create_events_plot2(o,
                                                  all_behaviors=[self.pj[ETHOGRAM][idx]["code"] for idx in
                                                                 sorted_keys(self.pj[ETHOGRAM])],
                                                  all_subjects=[NO_FOCAL_SUBJECT] + [self.pj[SUBJECTS][idx]["name"] for
                                                                                     idx in
                                                                                     sorted_keys(self.pj[SUBJECTS])],
    ...
    def edit_project(self, mode):
        ...
        if self.observationId:
            QMessageBox.warning(self, programName, ("Você deve fechar a observação atual antes de criar"
                                                    " um novo projeto ou modificando o projeto atual."))
            return
        ...
        if mode == NEW:
            ...
            # mesas vazias da janela principal
    ...
    def save_project_as_activated(self):
        ...
        """
        if QT_VERSION_STR[0] == "4":
            projectNewFileName, filtr = QFileDialog(self).getSaveFileNameAndFilter(self, "Save project as",
                                                                                   os.path.dirname(
                                                                                       self.projectFileName),
                                                                                   "Projects file (*.eMOC);;All files (*)")
        else:
            projectNewFileName, filtr = QFileDialog(self).getSaveFileName(self, "Save project as",
                                                                          os.path.dirname(self.projectFileName),
                                                                          "Projects file (*.eMOC);;All files (*)")
        if not projectNewFileName:
            ...
            # add .eMOC if filter = 'Projects file (*.eMOC)'
            if filtr == "Projects file (*.eMOC)" and os.path.splitext(projectNewFileName)[1] != ".eMOC":
                projectNewFileName += ".eMOC"
    ...
    def save_project_activated(self):
        ...
        if not self.projectFileName:
            if not self.pj["project_name"]:
                ...
                txt = self.pj['project_name'] + '.eMOC'
            os.chdir(os.path.expanduser("~"))
            if QT_VERSION_STR[0] == "4":
                self.projectFileName, filtr = QFileDialog(self).getSaveFileNameAndFilter(self, 'Save project', txt,
                                                                                         'Projects file (*.eMOC);;All files (*)')
            else:
                self.projectFileName, filtr = QFileDialog(self).getSaveFileName(self, 'Save project', txt,
                                                                                'Projects file (*.eMOC);;All files (*)')
            ...
            # add .eMOC if filter = 'Projects file (*.eMOC)'
            if filtr == 'Projects file (*.eMOC)' and os.path.splitext(self.projectFileName)[1] != '.eMOC':
                self.projectFileName += '.eMOC'
    ...
    def liveTimer_out(self):
        ...
        # extract State events
        StateBehaviorsCodes = [self.pj[ETHOGRAM][x][BEHAVIOR_CODE] for x in
                               [y for y in self.pj[ETHOGRAM] if 'State' in self.pj[ETHOGRAM][y][TYPE]]]
        ...
        for idx in self.pj[SUBJECTS]:
            ...
            self.currentStates[idx] = []
            for sbc in StateBehaviorsCodes:
                if len([x[pj_obs_fields[BEHAVIOR_CODE]] for x in self.pj[OBSERVATIONS][self.observationId][EVENTS] if
                        x[pj_obs_fields['subject']] == self.pj[SUBJECTS][idx]['name'] and x[
                            pj_obs_fields['code']] == sbc and x[
                            pj_obs_fields['time']] <= currentTime]) % 2:  # test if odd
                    self.currentStates[idx].append(sbc)
...
""".format(subjectsNum=subjectsNum, subjectsMin=subjectsMin, subjectsMax=subjectsMax)
    ...
    def switch_playing_mode(self):
        ...
        # go to frame by frame mode
            ...
            if list(self.fps.values())[0] == 0:
                logging.warning(
                    "The frame per second value is not available. Frame-by-frame mode will not be available")
                QMessageBox.critical(None, programName,
                                     "The frame per second value is not available. Frame-by-frame mode will not be available",
                                     QMessageBox.Ok | QMessageBox.Default, QMessageBox.NoButton)
                self.actionFrame_by_frame.setChecked(False)
            ...
            if len(set(self.fps.values())) != 1:
                logging.warning(
                    "The frame-by-frame mode will not be available because the video files have different frame rates")
            ...
            if self.second_player():
                if list(self.fps.values())[0] != list(self.fps2.values())[0]:
                    logging.warning(
                        "The frame-by-frame mode will not be available because the video files in player #1 and player #2 have different frame rates")
                    QMessageBox.warning(self, programName, ("The frame-by-frame mode will not be available"
            ...
            globalTime = (sum(self.duration[0: self.media_list.index_of_item(
                self.mediaplayer.get_media())]) + self.mediaplayer.get_time())
            ...
            if self.second_player():
                globalTime2 = (sum(self.duration2[0: self.media_list2.index_of_item(
                    self.mediaplayer2.get_media())]) + self.mediaplayer2.get_time())
    ...
    def add_event(self):
        ...
        if editWindow.exec_():  # button OK
            ...
            for obs_idx in self.pj[ETHOGRAM]:
                if self.pj[ETHOGRAM][obs_idx]["key"] == editWindow.cobCode.currentText():
    ...
    def edit_event(self):
        ...
        if self.twEvents.selectedItems():
            ...
            if self.timeFormat == HHMMSS:
                editWindow.dsbTime.setVisible(False)
                editWindow.teTime.setTime(
                    QtCore.QTime.fromString(seconds2time(self.pj[OBSERVATIONS][self.observationId][EVENTS][row][0]),
                                            HHMMSSZZZ))
            ...
            if self.pj[OBSERVATIONS][self.observationId][EVENTS][row][EVENT_SUBJECT_FIELD_IDX] in sortedSubjects:
                editWindow.cobSubject.setCurrentIndex(sortedSubjects.index(
                    self.pj[OBSERVATIONS][self.observationId][EVENTS][row][EVENT_SUBJECT_FIELD_IDX]))
            else:
                QMessageBox.warning(self, programName,
                                    "The subject <b>{}</b> does not exist more in the subject's list".
                                    format(self.pj[OBSERVATIONS][self.observationId][EVENTS][row][
                                               EVENT_SUBJECT_FIELD_IDX]))
            ...
            if self.pj[OBSERVATIONS][self.observationId][EVENTS][row][EVENT_BEHAVIOR_FIELD_IDX] in sortedCodes:
                editWindow.cobCode.setCurrentIndex(
                    sortedCodes.index(self.pj[OBSERVATIONS][self.observationId][EVENTS][row][EVENT_BEHAVIOR_FIELD_IDX]))
            else:
            ...
            editWindow.leComment.setPlainText(
                self.pj[OBSERVATIONS][self.observationId][EVENTS][row][EVENT_COMMENT_FIELD_IDX])
            ...
            if editWindow.exec_():  # button OK
                ...
                for key in self.pj[ETHOGRAM]:
                    if self.pj[ETHOGRAM][key]["key"] == editWindow.cobCode.currentText():
                        ...
                        event["comment"] = editWindow.leComment.toPlainText()
                        event["row"] = row
                        event["original_modifiers"] = self.pj[OBSERVATIONS][self.observationId][EVENTS][row][
                            pj_obs_fields["modifier"]]
    ...
    def actionUser_guide_triggered(self):
        ...
        """
        userGuideFile = os.path.dirname(os.path.realpath(__file__)) + "/eMOC_user_guide.pdf"
        if os.path.isfile(userGuideFile):
            ...
            QDesktopServices.openUrl(QUrl("http://eMOC.readthedocs.org"))
    ...
    def click_signal_from_behaviors_coding_map(self, bcm_name, behavior_codes_list):
        ...
        for code in behavior_codes_list:
            try:
                ...
                QMessageBox.critical(self, programName,
                                     "The code <b>{}</b> of behavior coding map do not exists in ethogram.".format(
                                         code))
                return
    ...
    def actionAbout_activated(self):
        ...
        about_dialog.setIconPixmap(QPixmap(os.path.dirname(os.path.realpath(__file__)) + "/logo_emoc.png"))
        ...
        about_dialog.setInformativeText(("<b>{prog_name}</b> {ver} - {date}"
                                         ...
                                         "<br>"
                                         """eMOC is released under the <a href="http://www.gnu.org/copyleft/gpl.html">GNU General Public License</a><br>"""
                                         """See <a href="http://www.eMOC.unito.it">www.eMOC.unito.it</a> for more details.<br>"""
                                         "<br>"
                                         "The authors would like to acknowledge Sergio Castellano, Valentina Matteucci and Laura Ozella for their precious help."
                                         "<hr>"
                                         "How to cite eMOC:<br>"
                                         "Friard, O. and Gamba, M. (2016), eMOC: a free, versatile open-source event-logging software for video/audio "
    ...
    def timer_out(self, scrollSlider=True):
        ...
        if self.pj[OBSERVATIONS][self.observationId][TYPE] in [MEDIA]:
                ...
                if TIME_OFFSET_SECOND_PLAYER in self.pj[OBSERVATIONS][self.observationId]:
                    ...
                    if self.pj[OBSERVATIONS][self.observationId][TIME_OFFSET_SECOND_PLAYER] > 0:
                        ...
                        if mediaTime < self.pj[OBSERVATIONS][self.observationId][TIME_OFFSET_SECOND_PLAYER] * 1000:
                            ...
                            if self.mediaListPlayer.get_state() == vlc.State.Playing:
                                t1, t2 = self.mediaplayer.get_time(), self.mediaplayer2.get_time()
                                if abs((t1 - t2) - self.pj[OBSERVATIONS][self.observationId][
                                    TIME_OFFSET_SECOND_PLAYER] * 1000) >= 300:  # synchr if diff >= 300 ms
                                    self.mediaplayer2.set_time(int(t1 - self.pj[OBSERVATIONS][self.observationId][
                                        TIME_OFFSET_SECOND_PLAYER] * 1000))
                                self.mediaListPlayer2.play()
                    ...
                    if self.pj[OBSERVATIONS][self.observationId][TIME_OFFSET_SECOND_PLAYER] < 0:
                        ...
                        if mediaTime2 < abs(
                            ...
                            if self.mediaListPlayer2.get_state() == vlc.State.Playing:
                                t1, t2 = self.mediaplayer.get_time(), self.mediaplayer2.get_time()
                                if abs((t2 - t1) + self.pj[OBSERVATIONS][self.observationId][
                                    TIME_OFFSET_SECOND_PLAYER] * 1000) >= 300:  # synchr if diff >= 300 ms
                                    self.mediaplayer.set_time(int(t2 + self.pj[OBSERVATIONS][self.observationId][
                                        TIME_OFFSET_SECOND_PLAYER] * 1000))
                                self.mediaListPlayer.play()

            currentTimeOffset = Decimal(currentTime / 1000) + Decimal(
                ...
                if abs(self.mediaTotalLength - totalGlobalTime / 1000) > 10:
                    ...
                    if not self.wrongTimeResponse:
                        self.wrongTimeResponse = dialog.MessageDialog(programName,
                                                                      ...
                                                                      " and re-encode your media file with the eMOC tool<br>"
                ...
                if self.mediaListPlayer.get_state() == vlc.State.Playing or self.mediaListPlayer.get_state() == vlc.State.Paused:
                    msg = "{media_name}: <b>{time} / {total_time}</b>".format(media_name=mediaName,
                                                                              time=self.convertTime(
                                                                                  Decimal(mediaTime / 1000)),
                                                                              total_time=self.convertTime(
                                                                                  Decimal(self.mediaTotalLength)))
            ...
            if (self.memMedia and mediaName != self.memMedia) or (
                    self.mediaListPlayer.get_state() == vlc.State.Ended and self.timer.isActive()):
        ...
        if self.pj[ETHOGRAM]:
            for idx in sorted_keys(self.pj[ETHOGRAM]):
                ...
                for col in behav_fields_in_mainwindow.keys():
                    ...
                    self.twEthogram.setItem(self.twEthogram.rowCount() - 1, field, QTableWidgetItem(str(self.pj[ETHOGRAM][idx][col])))
        if self.twEthogram.rowCount() < len(self.pj[ETHOGRAM].keys()):
            self.dwEthogram.setWindowTitle(
                "Etograma (filtered {0}/{1})".format(self.twEthogram.rowCount(), len(self.pj[ETHOGRAM].keys())))

            if self.observationId:
                self.pj[OBSERVATIONS][self.observationId]["filtered behaviors"] = behaviorsToShow
        else:
            self.dwEthogram.setWindowTitle("Etograma")
    ...
    def writeEvent(self, event, memTime):
        ...
        try:
            ...
            if "row" not in event and self.checkSameEvent(self.observationId, memTime, self.currentSubject,
                                                          event["key"]):
                _ = dialog.MessageDialog(programName, "O mesmo evento já existe (mesmo tempo, código de comportamento e assunto).", [OK])
                return
            ...
            if "row" in event:
                # modifying event
                self.pj[OBSERVATIONS][self.observationId][EVENTS][event["row"]] = [memTime, subject, event["key"], event[TYPE], modifier_str, event['category']]
            else:
                # add event
                self.pj[OBSERVATIONS][self.observationId][EVENTS].append(
                    [memTime, subject, event["key"], event[TYPE], modifier_str, event["category"]])

            # sort events in pj
            self.pj[OBSERVATIONS][self.observationId][EVENTS].sort()

            #event send
            Event_subject = {"ID": self.observationId, "Event_Subject": self.pj[OBSERVATIONS][self.observationId]}

            # reload all events in tw
            self.loadEventsInTW(self.observationId)
            item = self.twEvents.item(
                [i for i, t in enumerate(self.pj[OBSERVATIONS][self.observationId][EVENTS]) if t[0] == memTime][0], 0)
            self.twEvents.scrollToItem(item)
            self.projectChanged = True
    ...
    def fill_lwDetailed(self, obs_key, memLaps):
        ...
        for idx in self.pj[ETHOGRAM]:
            if self.pj[ETHOGRAM][idx]["key"] == obs_key:
                ...
                code_descr = self.pj[ETHOGRAM][idx]["key"]
        ...
        dbc = dialog.DuplicateBehaviorCode(
            "The <b>{}</b> key codes more behaviors.<br>Choose the correct one:".format(obs_key), items)
        if dbc.exec_():
            code = dbc.getCode()
            if code:
    ...
    def delete_all_events(self):
        ...
        if dialog.MessageDialog(programName, "Confirm the deletion of all events in the current observation?",
                                [YES, NO]) == YES:
            self.pj[OBSERVATIONS][self.observationId][EVENTS] = []
            self.projectChanged = True
            self.loadEventsInTW(self.observationId)
    ...
    def click_signal_find_in_events(self, msg):
        ...
        for event_idx, event in enumerate(self.pj[OBSERVATIONS][self.observationId][EVENTS]):
            ...
            if ((not self.find_dialog.cbFindInSelectedEvents.isChecked()) or (
                    self.find_dialog.cbFindInSelectedEvents.isChecked()
        ...
        if msg != "FIND_FROM_BEGINING":
            if dialog.MessageDialog(programName, "<b>{}</b> not found. Search from beginning?".format(
                    self.find_dialog.findText.text()),
                                    [YES, NO]) == YES:
    ...
    def export_tabular_events(self):
        ...
        for obsId in selectedObservations:
            ...
            r, msg = export_observation.export_events(parameters, obsId, self.pj[OBSERVATIONS][obsId],
                                                      self.pj[ETHOGRAM],
    ...
    def create_behavioral_strings(self, obsId, subj, plot_parameters):
        ...
        for event in eventsWithStatus:
            if event[EVENT_SUBJECT_FIELD_IDX] == subj or (
                    subj == NO_FOCAL_SUBJECT and event[EVENT_SUBJECT_FIELD_IDX] == ""):
    ...
    def export_string_events(self):
        ...
        for use with Behatrix (see http://www.eMOC.unito.it/pages/behatrix)
        """
    ...
    def transitions_matrix(self, mode):
        ...
        * frequencies_after_behaviors
        ...
        for subject in plot_parameters["selected subjects"]:
            ...
            sequences, observed_behaviors = transitions.behavioral_strings_analysis(strings_list,
                                                                                    self.behaviouralStringsSeparator)
            ...
            if flagMulti:
                try:
                    ...
                    if os.path.isfile(nf) and not flag_overwrite_all:
                        answer = dialog.MessageDialog(programName,
            ...
            with open(fileName, "r") as infile:
                try:
                    ...
                    QMessageBox.information(self, programName,
                                            "Error during dot script creation.\n{}".format(str(sys.exc_info()[0])))
    ...
    def closeEvent(self, event):
        ...
        if self.ffmpeg_recode_process:
            QMessageBox.warning(self, programName, "eMOC is re-encoding/resizing a video. Please wait before closing.")
            event.ignore()
        ...
        if self.playerType == VLC:
            ...
            if self.playMode == FFMPEG:
                ...
                if self.FFmpegGlobalFrame * (1000 / list(self.fps.values())[0]) >= sum(self.duration):
                    ...
                    logging.debug(
                        "FFmpegGlobalFrame {}  sum duration {}".format(self.FFmpegGlobalFrame, sum(self.duration)))
                ...
                if self.second_player():
                    ...
                    if self.FFmpegGlobalFrame2 * (1000 / list(self.fps2.values())[0]) >= sum(self.duration2):
                        ...
                        logging.debug("FFmpegGlobalFrame2 {}  sum duration2 {}".format(self.FFmpegGlobalFrame2,
                                                                                       sum(self.duration2)))
...
if __name__ == "__main__":
    ...
    # splashscreen, exibir imagem de espera para inicialização da aplicação PyQt5
    if (not options.nosplashscreen):
        start = time.time()
        splash = QSplashScreen(QPixmap(os.path.dirname(
            os.path.realpath(__file__)) + "/splash.png"))  # imagem de 'progresso que aparece no começo da aplicação'
        splash.show()
        splash.raise_()
    ...
    if vlc.dll is None:
        ...
        QMessageBox.critical(None, programName,
                             "Esse programa requer o VLC media player.<br>Baixe em http://www.videolan.org/vlc",
                             QMessageBox.Ok | QMessageBox.Default, QMessageBox.NoButton)
        sys.exit(1)

    availablePlayers.append(VLC)

    logging.debug("VLC versão {}".format(vlc.libvlc_get_version().decode("utf-8")))
    if vlc.libvlc_get_version().decode("utf-8") < VLC_MIN_VERSION:
        QMessageBox.critical(None, programName, ("O VLC media player está desatualizado ({}).<br>"
                                                 "Acesse http://www.videolan.org/vlc é baixe a nova atualização").format(
            vlc.libvlc_get_version()),
                             QMessageBox.Ok | QMessageBox.Default, QMessageBox.NoButton)

        logging.critical(("O media player VLC parece um pouco velho... ({})."
                          "Acesse http://www.videolan.org/vlc é baixe a nova atualização").format(vlc.libvlc_get_version()))
        sys.exit(2)
    ...
    if not ret:
        QMessageBox.critical(None, programName,
                             "FFmpeg não está disponível.<br>Acesse http://www.ffmpeg.org e baixe agora",
                             QMessageBox.Ok | QMessageBox.Default, QMessageBox.NoButton)
        sys.exit(3)
    else:
        ffmpeg_bin = msg

    # check matplotlib, api para calculos e graficos
    if not FLAG_MATPLOTLIB_INSTALLED:
        QMessageBox.warning(None, programName,
                            ("Algumas funções (eventos de plotagem e espectrograma) requerem o módulo Matplotlib."
                             """<br>Veja em <a href="http://matplotlib.org">http://matplotlib.org</a>"""),
                            QMessageBox.Ok | QMessageBox.Default, QMessageBox.NoButton)

    app.setApplicationName(programName)
    __window__ = MainWindow(availablePlayers, ffmpeg_bin)
    ...
    if project_to_open:
        ...
        if "error" in pj:
            logging.debug(pj["error"])
            QMessageBox.critical(__window__, programName, pj["error"])
        else:
            if msg:
                QMessageBox.information(__window__, programName, msg)
            __window__.load_project(project_path, project_changed, pj)

    if observation_to_open and "error" not in pj:
        r = __window__.load_observation(observation_to_open)
        if r:
            QMessageBox.warning(None, programName,
                                "Erro ao abrir a observação: <b>{}</b><br>{}".format(observation_to_open,
                                                                                    r.split(":")[1]),
                                QMessageBox.Ok | QMessageBox.Default, QMessageBox.NoButton)
    ...
    app.focusChanged.connect(__window__.changedFocusSlot)

    if not options.nosplashscreen:
        splash.finish(__window__)
