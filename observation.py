...
class AssignConverter(QDialog):
    ...
    def __init__(self, columns, converters, col_conv):
        ...
        self.setWindowTitle("Conversores")

        self.vbox = QVBoxLayout()

        self.label = QLabel()
        self.label.setText("Atribuir conversor a coluna")
...
class Observation(QDialog, Ui_Form):
    ...
    def tw_data_files_cellDoubleClicked(self, row, column):
        ...
        if column == PLOT_DATA_CONVERTERS_IDX:
            if self.tw_data_files.item(row, PLOT_DATA_COLUMNS_IDX).text():
                ...
                QMessageBox.critical(self, programName, "Selecione as colunas para traçar (tempo, valor)")
    ...
    def add_data_file(self, flag_path=True):
        ...
        if self.tw_data_files.rowCount() >= 2:
            QMessageBox.warning(self, programName , ("Ainda não é possível plotar mais de 2 dados externos"
                                                     "Esta limitação será removida no futuro."))
            return

        QMessageBox.warning(self, programName, "Está função é experimental.<br>Por favor relate qualquer bug")
        ...
        if file_name:
            ...
            if not r["homogeneous"]: # not all rows have 2 columns
                QMessageBox.critical(self, programName , "Este arquivo não contém um número constante de colunas")
                return

            header = self.return_file_header(file_name)
            if header:
                text, ok = QInputDialog.getText(self, "Arquivos de dados: {}".format(Path(file_name).name),
                                                ("Este arquivo contém {} colunas. 2 são necessários para o enredo.<br>"
                                                 "<pre>{}</pre><br>"
                                                 "Insira os índices da coluna para traçar (hora, valor) separados por vírgula").format(r["fields number"], header))
                if ok:
                    if len(text.split(",")) != 2:
                        QMessageBox.critical(self, programName , "Indique apenas dois índices de coluna")
                        return
                    ...
                    for col in text.split(","):
                        try:
                            ...
                            QMessageBox.critical(self, programName , "<b>{}</b> não parece ser um índice de coluna".format(col))
                            return
                        if col_idx < 0 or col_idx > r["fields number"]:
                            QMessageBox.critical(self, programName , "<b>{}</b> não é um índice de coluna válido".format(col))
                            return
                else:
                    return
            else:
    ...
    def check_parameters(self):
        ...
        if not is_numeric(self.leTimeOffset.text()):
            QMessageBox.warning(self, programName , "<b>{}</b> não é reconhecido como um formato de deslocamento de hora válido".format(self.leTimeOffset.text()))
            return False
            ...
            if self.twIndepVariables.item(row, 1).text() == NUMERIC:
                if self.twIndepVariables.item(row, 2).text() and not is_numeric( self.twIndepVariables.item(row, 2).text() ):
                    QMessageBox.critical(self, programName,
                                         "The <b>{}</b> variável deve ser numérica!".format(self.twIndepVariables.item(row, 0).text()))
                    return False

        # check if observation id not empty
        if not self.leObservationId.text():
            QMessageBox.warning(self, programName , "A <b>observação id</b> é obrigatório e deve ser único!" )
            return False
        ...
        if (self.mode == "new") or (self.mode == "edit" and self.leObservationId.text() != self.mem_obs_id):
            if self.leObservationId.text() in self.pj[OBSERVATIONS]:
                QMessageBox.critical(self, programName,
                                     "A observação id <b>{0}</b> já é usado!<br>{1}<br>{2}".format(self.leObservationId.text(),
        ...
        if self.tabProjectType.currentIndex() == 0 and not self.twVideo1.rowCount():
            QMessageBox.critical(self, programName , "Adicione um arquivo de mídia no primeiro media player!" )
            return False
        ...
        for row in range(self.tw_data_files.rowCount()):
            if not is_numeric(self.tw_data_files.item(row, PLOT_DATA_TIMEOFFSET_IDX).text()):
                QMessageBox.critical(self, programName,
                                     ("O valor inicial do arquivo de dados externo <b>{}</b> não é reconhecido como um valor numérico.<br>"
                                     "Use o número decimal de segundos (e.g. -58.5 or 32)").format(
        ...
        for row in range(self.twIndepVariables.rowCount()):
            if self.twIndepVariables.item(row, 1).text() == NUMERIC:
                if self.twIndepVariables.item(row, 2).text() and not is_numeric( self.twIndepVariables.item(row, 2).text() ):
                    QMessageBox.critical(self, programName,
                                         "<b>{}</b> variável deve ser numérica!".format(self.twIndepVariables.item(row, 0).text()))
                    return False
    ...
    def add_media(self, n_player, flag_path):
        ...
        if n_player == PLAYER2 and self.twVideo1.rowCount() > 1:
            QMessageBox.critical(self, programName, ("Ainda não é possível reproduzir uma segunda mídia "
                                                     "quando mais mídia é carregada no primeiro media player"))
            return
        ...
        if file_paths:
            for file_path in file_paths:
                if not self.check_media(n_player, file_path, flag_path):
                    QMessageBox.critical(self, programName, "<b>{file_path}</b> arquivo não parece ser um arquivo de mídia.".format(
                                 file_path=file_path))
    ...
    def add_media_to_listview(self, nPlayer, fileName, fileContentMD5):
        ...
        if not self.twVideo1.rowCount() and nPlayer == PLAYER2:
            QMessageBox.critical(self, programName, "Adicione o primeiro arquivo de mídia ao Player #1")
            return False
    ...
    def remove_data_file(self):
        ...
        if self.tw_data_files.selectedIndexes():
            ...
            QMessageBox.warning(self, programName, "Selecione um arquivos de dados")
