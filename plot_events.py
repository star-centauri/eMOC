...
def default_value(ethogram, behav, param):
    ...
    """
    default_value_ = 0
    if ({ethogram[idx]["type"] for idx in ethogram if ethogram[idx]["key"] == behav} == {"Point event"}
...
def create_events_plot2_new(pj, #projeto atual
                            selected_observations, #observações dos especialistas
                            parameters, # os parametos observados
